...
export interface ThreeDLetterSwapProps
  ...
  /** Optional ref to an external element that triggers the hover animation */
  parentHoverRef?: React.RefObject<HTMLElement | null>;
}
...
const ThreeDLetterSwap: React.FC<ThreeDLetterSwapProps> = ({
  ...
  useEffect(() => {
    if (!parentHoverRef?.current || playOnScroll) return;
    const el = parentHoverRef.current;
    const onEnter = () => handleHoverStart();
    const onLeave = () => handleHoverEnd();
  ...
  }, [parentHoverRef, handleHoverStart, handleHoverEnd, playOnScroll]);
  ...
  if (
    ...
    return (
      <Tag
        ...
        onMouseEnter={parentHoverRef ? undefined : handleHoverStart}
        onMouseLeave={parentHoverRef ? undefined : handleHoverEnd}
