    ...
    afterChange: [
      async ({ doc }) => {
        ...
        try {
          ...
          await fetch(
            `${siteUrl}/api/revalidate?path=/projects/${doc.slug}&secret=${process.env.REVALIDATION_SECRET}`,
          )

          // Also revalidate the projects list page
          await fetch(
            `${siteUrl}/api/revalidate?path=/projects&secret=${process.env.REVALIDATION_SECRET}`,
          )
